...
export default function PreviewModal({ 
    ...
    // 根据类型生成正确的图片URL
    const getImageUrl = () => {
        if (previewImage.type === 'icon' && previewImage.collection_id) {
    ...
    return (
        <AnimatePresence>
            <motion.div
                className="fixed inset-0 bg-black bg-opacity-75 flex items-start sm:items-center justify-center z-50 p-4 pt-8 sm:pt-4 overflow-y-auto"
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
            ...
            >
                <motion.div
                    className="bg-white rounded-xl w-full max-w-xs sm:max-w-lg md:max-w-2xl lg:max-w-4xl xl:max-w-5xl max-h-[calc(100vh-4rem)] sm:max-h-[90vh] overflow-auto shadow-2xl mx-4 my-auto"
                ...
                >
                    ...
                    <div className="flex justify-between items-center p-4 sm:p-6 border-b border-gray-200">
                        <h2 className="text-lg sm:text-xl font-bold text-gray-800 truncate pr-4">{previewImage.name}</h2>
                        <button
                            onClick={handleClose}
                        ...
                        <div className="flex flex-col lg:flex-row gap-4 sm:gap-6">
                            ...
                            <div className="flex-1 flex justify-center items-center bg-gray-50 rounded-lg p-3 sm:p-4 min-h-[200px] sm:min-h-[300px]">
                                {previewImage.type === 'trophy' ? (
                                    ...
                                    >
                                        <div className="text-white text-sm sm:text-lg font-bold text-center px-2 sm:px-4"
                            ...
                            <div className="w-full lg:w-80 xl:w-96 bg-gray-50 rounded-lg p-3 sm:p-4">
                                <h3 className="text-base sm:text-lg font-semibold text-gray-800 mb-3 sm:mb-4">获取条件</h3>
                                ...
                                {conditionLoading ? (
                                    <div className="flex items-center justify-center py-6 sm:py-8">
                                        ...
                                        <span className="ml-2 sm:ml-3 text-sm sm:text-base text-gray-600">加载中...</span>
                                    </div>
                                ) : condition ? (
