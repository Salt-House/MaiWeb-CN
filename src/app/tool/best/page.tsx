...
export default function BestPage() {
    ...
    useEffect(() => {
        setAccounts([{ server: "1", nickname: "TokyaSZK", identifier: "TokyaSzk", from: "none" },
        { server: "2", nickname: "ＴＯＫＩＹＡ～！", identifier: "389590066847940", from: "none" }])
    ...
    const GetBest50 = () => {
        ...
        if (nowFrom == "divingfish") {
            ...
            fetch("https://dev.maimai.moe/api/maimai/divingfish/bests?username=TokyaSZK", requestOptions)
                .then((response) => response.text())
                .then((result) => {
                    const data = JSON.parse(result);
                    if (data.rating) {
            ...
            <div className="w-[900px] p-5 flex flex-col justify-center items-center mx-auto">
                ...
                <div className="w-[900px] flex flex-row justify-center flex-wrap mx-auto">
                    {best35 && best35.map((song: any, index: number) => {
                        return (
                            <div key={index} className="w-[160px] h-48 break-words p-2 m-2 border border-gray-300 rounded-lg shadow-md hover:scale-105 hover:shadow-xl duration-300 ease-in-out backdrop-filter backdrop-blur-lg bg-white bg-opacity-30">
                ...
                <div className="w-[900px] flex flex-row justify-center flex-wrap mx-auto">
                    {best15 && best15.map((song: any, index: number) => {
                        return (
                            <div key={index} className="w-[160px] h-48 break-words p-2 m-2 border border-gray-300 rounded-lg shadow-md hover:scale-105 hover:shadow-xl duration-300 ease-in-out">
                                <h1 className="w-full h-10 text-lg font-bold truncate">{song.song_name}</h1>
                                <h2 className="text-md">{song.level}</h2>
                                <h3 className="text-sm text-gray-600">{song.achievements}</h3>
                                <img className="size-10 mt-2" src={`${baseUrl}/jacket/${song.id}.png`} alt={song.song_name} />
                            </div>
                        );
                    })}
                </div>
