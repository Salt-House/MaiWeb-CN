...
import SvgStrokedText from "./SvgStrokedText"
...
export default function NeedItem(props: NeedItemProps) {
    ...
    const getStatusColor = (status?: number) => {
        switch (status) {
            ...
            case 1: return 'bg-blue-100 text-blue-600 border-blue-300';
            case 0: return 'bg-green-100 text-green-600 border-green-300';
    ...
    const handleAddComment = async () => {
        if (newComment.trim()) {
            ...
            try {
                ...
                if (data.success === true) { // 根据后端返回的success字段判断
        ...
        <div className="w-full h-auto min-h-[320px] max-w-sm rounded-xl shadow-lg bg-white transition-all duration-300 hover:shadow-xl hover:scale-105 overflow-hidden">
            ...
            <div className="p-6 pb-4">
                ...
                <div className="flex items-start justify-between mb-3">
                    <h1 className="text-xl font-bold text-gray-800 flex-1 pr-3 leading-tight">{props.title}</h1>
                    {props.priority && (
                        <div className={`w-3 h-3 rounded-full ${getPriorityColor(props.priority)} flex-shrink-0 mt-1`}
                            title={`优先级: ${props.priority}`}></div>
                    )}
                </div>
            ...
            <div className="px-6 pb-6">
                ...
                <div className="flex items-center justify-between mb-3">
                    {props.status !== undefined && (
                        <span className={`px-3 py-1 text-xs font-medium rounded-full border ${getStatusColor(props.status)}`}>
